...
Representative explicit-link evidence, 2026-08-26: the saved text-only **Welcome Series** Draft and **Abandoned Cart** Draft now each contain an explicit `Manage your email preferences: {% manage_preferences %}` line and an explicit `Unsubscribe: {% unsubscribe %}` line. Klaviyo’s saved preview resolves both tags to visible **Manage Preferences** and **Unsubscribe** hyperlinks, while each message remains **Draft**. The Welcome and Abandoned Cart previews were inspected directly; no test send, flow activation, SMS, discount, or campaign was performed. The other seven text-only Draft emails remain protected by Klaviyo’s documented automatic basic unsubscribe footer and confirmed account-level consent pages; because no activation is authorized, this record does not claim their tag-level rendering was individually previewed.

Expanded explicit-link evidence, 2026-08-26: the saved text-only **Back-in-Stock Alert** Draft now also contains the same visible `{% manage_preferences %}` and `{% unsubscribe %}` controls. Its direct saved preview resolves the controls as **Manage Preferences** and **Unsubscribe** hyperlinks. The message remains **Draft**, keeps `hello@wenestwell.com` as sender/reply-to, and retains its factual stock-availability language; no test send, SMS, discount, flow activation, or stock-reservation claim was introduced.
