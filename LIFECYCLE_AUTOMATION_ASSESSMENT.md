...
Final post-configuration Draft audit, 2026-08-26: the live Klaviyo Flows list directly showed all nine Nestwell automations as **Draft** with zero conversions, including the now configured Abandoned Cart and Product Abandonment recovery paths. The list showed their expected custom triggers—**Nestwell Added to Cart** and **Nestwell Product Viewed**, respectively. Customer Win-Back remains a Draft with no message action by owner direction pending marketing-consent and new-order filters. No live, manual, or SMS flow is present in the Nestwell automation set.
...
Abandoned Cart email body configuration, 2026-08-25: a text-only Draft email was added only to the consented, four-hour, zero-order recovery-eligible path. Its name is “Cart reminder · Your Nestwell picks,” its subject is “Your Nestwell cart is still waiting,” and its sender/reply-to is `hello@wenestwell.com`. The saved body is low-pressure, asks the recipient to check current availability, options, and pricing before checkout, and directs questions to `support@wenestwell.com`. The direct post-save editor readback shows **Draft** and no active delivery; no discount, urgency, SMS, or activation was added.
...
Product Abandonment repeat-interest safeguard, 2026-08-25: direct Klaviyo trigger readback confirms the Profile filters now require both marketing eligibility and **Nestwell Product Viewed at least 2 times in the last 7 days**. This is a measurable stronger-interest threshold and does not itself add any message, delay, discount, SMS, or activation. A post-delay cart/purchase suppression step remains mandatory before a recovery email can be considered.

Product Abandonment suppression safeguard, 2026-08-25: the Draft flow now follows **Nestwell Product Viewed → marketing consent + at least 2 views in 7 days → Wait 1 day → Split**. The first, recovery-eligible split path requires both **Nestwell Added to Cart zero times since starting this flow** and **Placed Order zero times since starting this flow**; every other profile exits via the otherwise path. The flow remains Draft-only and message-free: no email, SMS, discount, or activation has been added.

Product Abandonment placement correction, 2026-08-25: an initially added unsent Draft email appeared above the one-day delay and split, so it was deleted immediately rather than completed. The current direct canvas readback has no email action and retains only the trigger, one-day wait, and safeguarded split. No customer message, SMS, discount, or activation occurred. A correctly placed Draft email may be added only below the first, recovery-eligible split path in a later controlled step.

Product Abandonment email body configuration, 2026-08-25: the corrected **Product reminder · A closer look** text-only email was added only below the first post-delay split path. It has the factual subject “A closer look at your Nestwell picks,” is sent/replied from `hello@wenestwell.com`, and reminds recipients to check the current product page for availability, options, and pricing. The body states that recipients who added an item to cart or completed an order since will not receive the reminder and routes questions to `support@wenestwell.com`. The direct post-save preview shows **Draft** with no discount, SMS, urgency claim, or activation.

Win-Back safeguard and email configuration, 2026-08-26: the Draft now follows **Placed Order → subscribed email-marketing profile → Wait 75 days → Split**. The first, re-engagement-eligible path requires **Placed Order zero times since starting this flow**; everyone else exits. A text-only Draft email, “Win-back · A quiet return,” was added only below that path with subject “A quiet reminder from Nestwell,” `hello@wenestwell.com` as sender/reply-to, and a factual low-pressure body that directs questions to `support@wenestwell.com`. Direct post-save content readback shows the body and **Draft** status. No discount, SMS, incentive, customer delivery, or activation was added.

Final lifecycle validation, 2026-08-26: Klaviyo’s direct Flows list readback shows exactly nine Nestwell automations, all with **Draft** status and zero conversions: Abandoned Cart, Back-in-Stock Alert, Browse Abandonment, Customer Thank You, Customer Win-Back, Post-Purchase Care, Product Abandonment, Review Request, and Welcome Series. Their displayed triggers match the documented design: custom Added to Cart / Product Viewed signals for recovery, explicit stock interest, Shopify order and fulfillment signals, and the double-opt-in Email List. No row is Live, no SMS flow is present, and all activation gates remain intact.

Unsubscribe activation gate, 2026-08-26: Klaviyo’s official guidance requires an unsubscribe link in every email and states that, when an email lacks an unsubscribe tag, it automatically appends a basic unsubscribe footer. Klaviyo also automatically supplies one-click unsubscribe headers for supported inboxes. Nestwell’s text-only Draft emails must be directly reviewed in the Klaviyo editor or preview before any activation decision to confirm that this account-level behavior is present or that an explicit `{% unsubscribe %}` tag is included. References: https://help.klaviyo.com/hc/en-us/articles/115006054267 and https://help.klaviyo.com/hc/en-us/articles/115005078267.

Unsubscribe and preference-page validation, 2026-08-26: direct Klaviyo account evidence at **Settings → Other → Consent pages** confirms that this account has a Preference page, Email unsubscribe page, and Global unsubscribe page (with an account-level global opt-out address). The representative Welcome Series text-only Draft editor was opened read-only. Its in-editor preview contains the factual consent statement that the recipient may update preferences or unsubscribe, and displays “Powered by Klaviyo.” The editor preview did **not** display a resolved unsubscribe hyperlink or an automatically appended footer; no test email was sent because the owner’s current instruction is Draft-only. Accordingly, this record does not claim a rendered per-email footer. The handling relied on at this stage is Klaviyo’s documented automatic basic-footer behavior for an email without an unsubscribe tag, together with the directly confirmed account-level preference/unsubscribe pages. Any future activation must either add explicit `{% unsubscribe %}` and `{% manage_preferences %}` tags to the relevant content or obtain fresh owner approval for a controlled send that verifies the rendered footer and links.

Representative explicit-tag evidence, 2026-08-26: the saved text-only **Welcome Series** Draft and **Abandoned Cart** Draft now each contain an explicit `Manage your email preferences: {% manage_preferences %}` line and an explicit `Unsubscribe: {% unsubscribe %}` line. The raw tags were directly read back in the editor before save, and each message remained **Draft**. No test send, flow activation, SMS, discount, or campaign was performed. This record does not claim a separately rendered post-save link preview for either email.

Expanded explicit-tag evidence, 2026-08-26: the saved text-only **Back-in-Stock Alert** Draft now also contains the same visible `{% manage_preferences %}` and `{% unsubscribe %}` controls. The raw tags were directly read back in the editor before save. The message remains **Draft**, keeps `hello@wenestwell.com` as sender/reply-to, and retains its factual stock-availability language; no test send, SMS, discount, flow activation, or stock-reservation claim was introduced. This record does not claim a separately rendered post-save link preview.

Further explicit-tag evidence, 2026-08-26: the saved text-only **Browse Abandonment** Draft and **Customer Thank You** Draft now each contain the same explicit preference and unsubscribe tags. The raw tags were directly read back in each editor before save. Both messages remain **Draft** with their established `hello@wenestwell.com` sender, low-pressure/factual content, and `support@wenestwell.com` support routing. No email was sent and no SMS, discount, campaign, or flow status was activated. This record does not claim separately rendered post-save link previews for those two messages.

Post-purchase explicit-tag evidence, 2026-08-26: the saved text-only **Post-Purchase Care** Draft now contains the explicit `{% manage_preferences %}` and `{% unsubscribe %}` controls. The raw tags were directly read back in the editor before save. The message remains **Draft**, retains `hello@wenestwell.com` as sender/reply-to, and preserves factual order-care and support guidance. No email was sent and no SMS, discount, campaign, or flow status was activated. This record does not claim a separately rendered post-save link preview.

Review-request rendered-link evidence, 2026-08-26: the saved text-only **Review Request** Draft now contains the explicit `{% manage_preferences %}` and `{% unsubscribe %}` controls, rendered as directly visible **Manage Preferences** and **Unsubscribe** hyperlinks in the saved post-edit preview. Its sender/reply-to remains `support@wenestwell.com`; the 14-day post-fulfillment timing, direct reply-feedback request, no-rating language, and no-reward safeguard remain unchanged. No email was sent and no SMS, discount, campaign, or flow status was activated.

Current explicit-tag coverage, 2026-08-26: seven of the nine Draft lifecycle messages have now been directly saved with explicit preference and unsubscribe tags: Welcome Series, Abandoned Cart, Back-in-Stock Alert, Browse Abandonment, Customer Thank You, Post-Purchase Care, and Review Request. The Review Request saved preview directly rendered both links; for the other six, this record only asserts that the raw tags were seen and saved while Draft. Customer Win-Back and Product Abandonment require a separate read-only canvas-path verification because their message actions sit below post-delay eligibility splits; no unverified action will be moved, recreated, or activated merely to force this footer check.
...
| --- | --- | --- | --- | --- |
| Welcome Series | Added to the double-opt-in Email List; no re-entry. | Immediate single message; no recovery pressure. | `hello@wenestwell.com`; factual brand introduction only. | Direct editor names preference/unsubscribe; account pages directly confirmed; footer inheritance is documented platform behavior, not rendered-preview evidence. |
| Abandoned Cart | Custom `Nestwell Added to Cart`; no re-entry; profile must be subscribed/can receive email marketing. | Wait 4 hours; exit path requires zero Shopify Placed Order since flow entry. | `hello@wenestwell.com`, support routing; factual cart reminder; no discount. | Same confirmed account-level pages and documented automatic-footer/one-click handling. |
| Browse Abandonment | Shopify Viewed Product; email-only route; no SMS action remains. | Recovery timing and profile eligibility configured; no purchase incentive. | `hello@wenestwell.com`; factual, low-pressure product reminder. | Same confirmed account-level pages and documented automatic-footer/one-click handling. |
| Product Abandonment | Custom `Nestwell Product Viewed`; no re-entry; marketing consent plus at least two views in seven days. | Wait 1 day; only if zero custom Added to Cart and zero Shopify Placed Order since entry. | `hello@wenestwell.com`; eligible branch only; no discount. | Same confirmed account-level pages and documented automatic-footer/one-click handling. |
| Back-in-Stock Alert | Explicit Subscribed to Back in Stock interest; no unsolicited recovery entry. | Availability-led notification only. | `hello@wenestwell.com`; factual availability message; no stock-reservation promise. | Same confirmed account-level pages and documented automatic-footer/one-click handling. |
| Post-Purchase Care | Shopify Placed Order; no re-entry. | Order-led service timing; not a recovery path. | `hello@wenestwell.com` with support routing; tracking/care/support information only. | Same confirmed account-level pages and documented automatic-footer/one-click handling. |
| Customer Thank You | Shopify Placed Order; no re-entry. | Order-led appreciation; no incentive or additional conversion request. | `hello@wenestwell.com` with support routing; factual thank-you only. | Same confirmed account-level pages and documented automatic-footer/one-click handling. |
| Review Request | Fulfilled Order; no re-entry. | Wait 14 days after fulfillment. | `support@wenestwell.com`; direct reply feedback request only; no platform review, rating request, or reward. | Same confirmed account-level pages and documented automatic-footer/one-click handling. |
| Customer Win-Back | Shopify Placed Order; no re-entry; profile must be subscribed/can receive email marketing. | Wait 75 days; only if zero Shopify Placed Order since flow entry. | `hello@wenestwell.com` with support routing; low-pressure factual return note; no discount. | Same confirmed account-level pages and documented automatic-footer/one-click handling. |
