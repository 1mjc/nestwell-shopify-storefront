  ...
  links: [httpBatchLink({
    ...
    headers() {
      try {
        ...
        const token = raw?.split(";").find(value => value.trim().startsWith(`${COOKIE_NAME}=`))?.trim().slice(`${COOKIE_NAME}=`.length);
...
hydrateRoot(root, <trpc.Provider client={trpcClient} queryClient={queryClient}><QueryClientProvider client={queryClient}><HydrationBoundary state={dehydratedState}><App /></HydrationBoundary></QueryClientProvider></trpc.Provider>);
