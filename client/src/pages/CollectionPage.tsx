...
const productRating = (product: Product) => {
  if (!product.rating?.value) return undefined;
  try {
    const parsed = JSON.parse(product.rating.value) as { value?: string | number };
    return parsed.value === undefined ? undefined : Number(parsed.value);
  } catch {
    const numeric = Number(product.rating.value);
...
export default function CollectionPage() {
  ...
  const products = useMemo(() => {
    ...
    next = next.filter(product => {
      ...
      const parsedRating = productRating(product);
  ...
  const hasRatings = ((query.data || []) as Product[]).some(product => productRating(product) !== undefined);
