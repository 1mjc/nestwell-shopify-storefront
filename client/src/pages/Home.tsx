...
export function Footer() { return <footer className="site-footer"><div className="footer-top"><div><Link href="/" className="wordmark">nestwell<span>·</span></Link><p>Small rituals for deeper rest and softer routines.</p></div><div><span className="eyebrow">Explore</span>{COLLECTIONS.slice(0, 4).map(collection => <Link key={collection} href={`/collections/${collectionMeta[collection].slug}`}>{collection}</Link>)}</div><div><span className="eyebrow">Customer care</span><Link href="/contact">Contact</Link><Link href="/policies/shipping-policy">Shipping &amp; delivery</Link><Link href="/policies/refund-policy">Returns &amp; refunds</Link><Link href="/policies/privacy-policy">Privacy</Link><Link href="/policies/terms-of-service">Terms of service</Link></div></div><div className="footer-bottom"><span>© {new Date().getFullYear()} Nestwell</span><span>Live catalogue · Secure checkout</span></div></footer>; }
